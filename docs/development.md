...
You can run the tests with `make test` and the linters by running `make lint`.
There are further make targets defined, see the `Makefile` for details. After
any changes to the Javascript code, you will need to re-build the frontend,
which you can do by running `make`. If you are working on the frontend code,
running `bun run dev` in the `frontend` folder will watch for file changes and
rebuild the Javascript bundle continuously.
