...
from beanquery.sources.beancount import (  # type: ignore[import-untyped]
    add_beancount_tables,
)
...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from fava.core.query import BaseColumn
    from fava.core.query import SerialisedQueryRowValue
...
class FavaBQLShell(BQLShell):  # type: ignore[misc]
    ...
    def run(self, entries: list[Directive], query: str) -> QueryResult | str:
        ...
        except (FavaShellError, CompilationError, ParseError) as exc:
    ...
    def execute_query_serialised(
        ...
        if isinstance(res, str):
            if res.startswith("ERROR"):
                raise FavaAPIError(res)
            return QueryResultText(res)
...
def serialise_query_result(
    ...
    dtypes = [COLUMNS[c.datatype](c.name) for c in types]
