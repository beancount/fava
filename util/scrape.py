# Original __author__ = "Martin Blais <blais@furius.ca>"
...
def callbk(url, response, response_contents, html_root, skipped_urls):
    print(response.code, url)

def main(): #(filename, callback, port, ignore_regexp, quiet=True, extra_args=None):
    """Run a web server on a Beancount file and scrape it.
    ...
    Returns:
      A set of all the processed URLs and a set of all the skipped URLs.
    """
    url_format = 'http://localhost:{}{{}}'.format(5000)
    ...
    # - Components views... well there are just too many, makes the tests
    #   impossibly slow. Just keep the A's so some are covered.
    url_lists = scrape_urls(url_format, callbk, ignore_regexp=None)
