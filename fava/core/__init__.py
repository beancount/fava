...
class FavaLedger():
    ...
    def quantize(self, value, currency):
        ...
        if not currency:
            value = locale.format(self._default_format_string,
                                  value, grouping=True)
            return value
        value = self.options['dcontext'].quantize(value, currency)
        value = locale.format(self._format_string, value, grouping=True)
        return value
