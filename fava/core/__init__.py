...
class FavaLedger():
    ...
    def load_file(self):
        ...
        locale.setlocale(locale.LC_NUMERIC, self.fava_options['locale'])
    ...
    def quantize(self, value, currency):
        ...
        if not currency:
            value = locale.format(self._default_format_string, value, grouping=True)
            return value
        value = locale.format(self._format_string,
            self.options['dcontext'].quantize(value, currency), grouping=True)
        return value
